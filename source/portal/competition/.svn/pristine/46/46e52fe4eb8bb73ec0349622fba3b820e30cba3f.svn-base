                            ...
                            <li data-bind="css: id == Kaggle.Current.userId ? &#39;you&#39; : id == $parent.teamLeaderId() ? &#39;leader&#39; : null" class="you"> 
                              ...
                              <div class="user-details">
                                ...
                                ?>
                                    ...
                                    <h4 data-bind="visible: id != $parent.teamLeaderId()" style="display: block;">
                                      <a href="javascript:;" onclick="team_eject('<?php echo $member['id_user']; ?>','<?php echo $member['display_name']; ?>')">踢出团队</a>
                                    </h4>
                                <?php 
                                }
                                ?>
...
//踢出团队
function team_eject(id_user,display_name){
	if(confirm('您确定要将“'+display_name+'”踢出团队吗？')){
		$.ajax({
			url:$('#competition_index').val()+"/index.php?ajax=competition&op=team_eject&time="+ new Date().getTime(),
			data:'id_user='+id_user,
			type:'POST',
			async:true,
			beforeSend: function(){
