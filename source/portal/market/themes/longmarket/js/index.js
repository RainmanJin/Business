...
function pop_div(width,height,title,id,val,defined){
	$(".Main").append('<div id="pop_div_'+id+'"><div style="margin-top:30%; text-align:center"><img src="'+$('#link_root').val()+'/images/ajax-loader.gif" width="30" height="30" /></div></div>');
	if(val){
		$(".Main").append('<input type="hidden" id="pop_div_'+id+'_val" value="'+val+'" />');
	}
	$("#pop_div_"+id).dialog({
		modal: true,				 
		autoOpen:false,
	...
	if(!defined){
		...
		$("#pop_div_"+id).load($('#market_index').val()+'/index.php?view=pop_'+id+"&time="+time);
	}
}
//弹出层按钮
...
function pop_loading(){
    var flag = 0;
	$(".Main").append('<div id="pop_loading"><p id="pop_loading_word">载入中...</p><p id="pop_loading_second">0</p></div>');
    $("#pop_loading").dialog({
    	bgiframe: true,
    	autoOpen: true,
        resizable: false,
...
function validate_change(){
	$('.validate_img').attr('src',$('#market_index').val()+'/index.php?ajax=validate&'+ Math.random());
}
